...
class Client():
    ...
    def request(self, route_path, data):
        try:
            ...
            raise RuntimeError('install the optional `requests` dependency for Client') from exc

        post_path = urljoin(self.base_url.rstrip('/') + '/', route_path.lstrip('/'))
        try:
            response = requests.post(post_path, json=data)
        except Exception as exc:
            raise safe_runtime_error('Client request failed', exc) from None
    ...
    def get_session_id(self):
        ...
        if response:
            ...
            raise RuntimeError("Initializing client fails.")
