...
class LTMemoryRecall(BaseRecall):
    ...
    def __call__(self, query):
        ...
        elements = [self.storage.get_memory_element_by_mid(mid) for mid in ranking_ids]
        self.last_retrieved_elements = elements
...
class MGMemoryRecall(BaseRecall):
    def __init__(self, config, **kwargs):
        ...
        self.skip_trigger = getattr(self.config, 'skip_trigger', False)
        if not self.skip_trigger:
    ...
    def reset(self):
        objs = [self.truncation, self.utilization, self.recall_retrieval, self.archival_retrieval]
        if self.trigger is not None:
            objs.append(self.trigger)
        self.__reset_objects__(objs)
    ...
    def __trigger_function_call__(self, text):
        ...
        for (func_name, func_args) in exe_list:
            try:
                ...
                print('Fail to execute Function [%s(%s)]: %s'% (func_name, func_args, redact_runtime_text(e)))
