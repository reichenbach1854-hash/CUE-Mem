...
except ImportError:  # pragma: no cover - optional evaluation dependency
...
from benchmark.paths import BENCHMARK_ROOT, DATA_ROOT, RESULT_ROOT
...
def bert_score(prediction: str, ground_truth: str):
    ...
    if bert_score_compute is None:
        raise RuntimeError('install the optional `bert-score` dependency for BERTScore')
    ...
    P, R, F1 = bert_score_compute(
        [prediction], [ground_truth], lang='en', verbose=False, rescale_with_baseline=True
    )
        ...
        if enable_llm_judge:
            ...
            while True:
                try:
                    ...
                    print(
                        f"\nWarning: LLM judge failed for sample "
                        f"{sample.get('sample_id', 'unknown')}: {redact_runtime_text(e)}"
                    )
                    print("Retrying...")
...
if __name__ == "__main__":
    ...
    for data_name in datasets:
        ...
        try:
            ...
            try:
                ...
                print(f"Error saving individual result: {redact_runtime_text(e)}")
                
        except Exception as e:
            print(f"Error evaluating dataset {data_name}: {redact_runtime_text(e)}")
            continue
    ...
    if args.all_datasets and len(all_individual_results) > 1:
        ...
        try:
            ...
            print(f"Error saving overall result: {redact_runtime_text(e)}")
